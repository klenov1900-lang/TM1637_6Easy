# TM1637_6Easy  

Высокопроизводительная библиотека для 6-разрядных светодиодных дисплеев на основе драйвера TM1637.  

В отличие от многих альтернатив, данная библиотека **специализирована для работы на платформах с неточными тактовыми генераторами**, такими как **клоны Arduino с керамическим резонатором**. Её алгоритмы и тайминги оптимизированы для стабильной работы без «плавающих» задержек, которые приводят к сбоям протокола при использовании `digitalWrite()` в других библиотеках.  

⚙️ Требования и совместимость

*   **Микроконтроллеры:** Библиотека разработана для **8-битных AVR-микроконтроллеров** (например, ATmega328P).  
*   **Платы:** Совместима с платами на основе указанных микроконтроллеров, включая **Arduino Uno, Arduino Nano**, а также их клоны.  
*   **Тактовая частота:** Код оптимизирован для работы на частоте **16 МГц**, что является стандартом для большинства плат AVR.  
*   **Особенность:** Благодаря использованию **прямого управления регистрами портов ввода-вывода**, библиотека сохраняет стабильность таймингов даже на платах-клонах с **керамическим резонатором**, где другие решения могут давать сбои.  
*   **Примечание:** Для работы на других платформах (ESP8266, ESP32, STM32) требуется дополнительная адаптация и не гарантируется.


## 📦 Установка

### Способ 1: Через менеджер библиотек Arduino IDE (рекомендуется)

1. Откройте Arduino IDE
2. Перейдите в **Скетч** → **Подключить библиотеку** → **Управлять библиотеками...**
3. В поиске введите `TM1637_6Easy`
4. Нажмите **Установить**

### Способ 2: Вручную

1. Скачайте ZIP-архив с [последней версией](https://github.com/klenov1900-lang/TM1637_6Easy/releases)
2. В Arduino IDE: **Скетч** → **Подключить библиотеку** → **Добавить .ZIP библиотеку...**
3. Выберите скачанный архив

### Способ 3: Через Git


https://github.com/klenov1900-lang/TM1637_6Easy.git

🔌 Подключение  
Пин TM1637	Arduino  
VCC	+5V  
GND	GND  
CLK	D2-D13, A0-A5  
DIO	D2-D13, A0-A5  
📊 Допустимые пины  
Пины	Статус	Примечание  
D2–D7	✅ МОЖНО	Цифровые пины PORTD  
D8–D13	✅ МОЖНО	Цифровые пины PORTB  
A0–A5	✅ МОЖНО	Аналоговые пины как цифровые (PORTC)  
D0, D1	❌ НЕЛЬЗЯ	Заняты UART  
A6, A7	❌ НЕЛЬЗЯ	Только аналоговый вход  

🎯 Рекомендуемые пины  

TM1637_6Easy display(8, 9, 3);    // CLK=D8, DIO=D9  
TM1637_6Easy display(4, 5, 3);    // CLK=D4, DIO=D5  
TM1637_6Easy display(A0, A1, 3);  // CLK=A0, DIO=A1  

🚀 Быстрый старт  
```bash
#include "TM1637_6Easy.h"

TM1637_6Easy display(8, 9, 3);  // CLK=D8, DIO=D9, яркость=3

void setup() {
    display.begin();          // Инициализация
    display.showNumber(2026); // Вывод числа
    display.update();         // Отправка на дисплей
}

void loop() {
    // ...
}
```
📚 Доступные функции  
Функция	Описание  
begin()	Инициализация (нормальный порядок 0,1,2,3,4,5)  
begin(1)	Инициализация (китайский порядок 2,1,0,5,4,3)  
showNumber(n)	Вывод целого числа (-99999..999999)  
setDotOn(pos)	Включить точку на позиции (0-5)  
setDotOff(pos)	Выключить точку на позиции (0-5)  
clearDots()	Очистить все точки  
setDots(mask)	Установить точки битовой маской (0x00-0x3F)  
setBrightness(level)	Установить яркость (0-7)  
clear()	Очистить буфер  
update()	Отправить буфер на дисплей  

📊 Примеры использования  
Вывод числа с двоеточием (часы)  
```bash
display.showNumber(1234);
display.setDots(0b00010100);  // Точки на позициях 2 и 4
display.update();
Мигающая точка
 
display.showNumber(1234);
if (dot_state) {
    display.setDots(0b00010000);  // Позиция 4
} else {
    display.clearDots();
}
display.update();
Счётчик секунд
 
void loop() {
    static uint32_t last = 0;
    if (millis() - last >= 1000) {
        last = millis();
        display.showNumber((millis() / 1000) % 100);
        display.update();
    }
}
```
✨ Особенности  
- **Direct Port I/O**: Использование прямых указателей на регистры портов AVR для минимизации задержек и стабильности протокола TM1637.
- **Прямой портовый I/O**: Минимальные накладные расходы и максимальная скорость протокола.  
- **Стабильность на клонах**: Специально адаптирована для корректной работы на платах с **керамическим резонатором**, где стандартные библиотеки с задержками на `digitalWrite()` часто дают сбои.  
- **Минимальный размер**: Только самое необходимое для работы с дисплеем.  
- **Поддержка любых пинов**: D2-D13, A0-A5.  

⚠️ Отказоустойчивость  
Библиотека игнорирует отсутствие подтверждения от дисплея (NACK). Если модуль выйдет из строя, программа не зависнет, а просто будет отправлять данные «в пустоту». Текущие значения останутся в буфере и будут отображены после замены модуля.  


📝 История версий:  
    Версия	   v1.0.1   
    Дата	   2026-09-05    
    Изменения   
   ``` bash 
   library.properties  "Bump version to 1.0.1"

```
     
📄 Лицензия  
    MIT License  

## 📧 Контакты  

- **Email:** gray_wolf19@mail.ru
- **GitHub:** [klenov1900-lang](https://github.com/klenov1900-lang)

⭐ Если библиотека вам полезна, поставьте звезду на GitHub! ⭐
